...
io.on('connection', (socket) => {
  ...
  socket.emit('message', { user: 'server', message: 'Welcome to the Void Threads!' })
  ...
  socket.on('join-room', (roomName) => {
    if (socket.user?.room) {
      ...
      io.to(socket.user.room).emit('message', { user: 'server', message: `${socket.user.username} has left the Thread` })
    ...
    socket.emit('message', { user: 'server', message: `You have joined the ${roomName} Thread` })
    socket.broadcast.to(roomName).emit('message', { user: 'server', message: `${socket.user.username} has joined the Thread` })
